...
# Create figures and axes for mainshock and second event
...
# Set origin time for second event if provided, otherwise default to 100s
if args.t0_2nd:
...
for i, prefix_path in enumerate(args.prefix_paths):
    df0 = pd.read_csv(f"{prefix_path}-energy.csv")  # Read CSV file containing energy data
    ...
    for j, event in enumerate(["mainshock", "second_event"]):
        if event == "mainshock":
            ...
            df = df0[(df0.index > t0_2nd[i]) & (df0.index < t0_2nd[i]+40)]  # Filter data for second event
            print(f"warning: Mw computed over {t0_2nd[i]}-{t0_2nd[i]+40} (avoiding contribution of small residual moment after rupture)")
            if df.empty:
        ...
        ax[j].plot(df.index.values - t0, df["seismic_moment_rate"] / 1e19, cols[i], label=f"{label} (Mw={Mw:.2f})")  # Plot moment rate

# Plot comparison data from Melgar et al., 2023 and USGS
for j, event in enumerate(["mainshock", "second_event"]):
    Melgar = np.loadtxt(f"../../ThirdParty/moment_rate_Melgar_et_al_{event}.txt")
    ax[j].plot(Melgar[:, 0], Melgar[:, 1], "k", label="Melgar et al., 2023")

    usgs = np.loadtxt(f"../../ThirdParty/moment_rate_usgs_{event}.txt")
    ax[j].plot(usgs[:, 0], usgs[:, 1]/1e19, "k:", label="USGS")

    ax[j].legend(frameon=False, loc="upper right")
    ax[j].set_xlim([0, 80] if event == "mainshock" else [0, 40])
