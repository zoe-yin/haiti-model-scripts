#!/Users/hyin/miniconda3/envs/seissol/bin/python

# #!/lrz/sys/spack/release/22.2.1/views/python/3.8.11-extended/bin/python
import vtk
...
for idt in lidt:
    ...
    for var in args.Data:
        ...
        if args.gaussian_smoothing:
            ...
            projDataNp_g = gaussian_filter(projDataNp, 1)
            probedData.append(projDataNp_g)

            print("Apllying taper")
            print(projDataNp_g.keys())
            # tapering = np.maximum(-63000000, 2670.0*0.41*9.8*np.minimum(-200.0,z))/(-63000000) 
            # projDataNp_gt = projDataNp_g * tapering
            # probedData.append(projDataNp_gt)
    ...
    if args.paraview_readable:
        ...
        writeNetcdf(f"gridded_asagi_{idt}", xyz, args.Data, probedData, False)